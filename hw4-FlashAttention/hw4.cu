...
__global__ void flash_attention_kenel(float *q, float *k, float *m, float *l, float *o, float *v, int d, float scalar);
...
void flash_attention(float *q, float *k, float *v, float *o) {
    ...
    for (int j = 0; j < grid_size; j++) {
        flash_attention_kenel<<< grid, block >>>(device_q, device_k + j * BLOCK_SIZE * d, device_m, device_l , device_o, device_v + j * BLOCK_SIZE * d, d , 1.0 / sqrt(d));
    }
...
__global__ void flash_attention_kenel(float *q, float *k, float *m, float *l, float *o, float *v, int d, float scalar){
    __shared__ float shared_q[BLOCK_SIZE * BLOCK_SIZE * 2];
    __shared__ float shared_k[BLOCK_SIZE * BLOCK_SIZE * 2];
    __shared__ float shared_v[BLOCK_SIZE * BLOCK_SIZE * 2];
    ...
    for(int i = 0 ; i < step; i++){
        ...
        shared_k[tx * d + (i * BLOCK_SIZE + ty)] = k[tx * d + (i * BLOCK_SIZE + ty)];
        shared_v[tx * d + (i * BLOCK_SIZE + ty)] = v[tx * d + (i * BLOCK_SIZE + ty)];
    }
    ...
    RowSum(lij , pij);
    __syncthreads();
    UpdateMiLiOi(shared_m, shared_l, o, mij, lij, pij, shared_v, d);
    __syncthreads();
...
__device__ void UpdateMiLiOi(float *shared_m, float *shared_l, float *o, float *mij, float *lij, float *pij, float *shared_v , int d) {
    ...
    for (int j = 0; j < d / BLOCK_SIZE; j++) {
        ...
        o[(ty + start * BLOCK_SIZE) * d + (j * BLOCK_SIZE + tx)] = (thread_l * exp(thread_m - new_m[ty]) * o[(ty + start * BLOCK_SIZE) * d + (j * BLOCK_SIZE + tx)] + exp(thread_mij - new_m[ty]) * pv) / new_l[ty];
    }
